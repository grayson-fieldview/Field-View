...
- **Key Features**:
    ...
    - **Auto Clock-Out (S32a)**: When a user crosses out of a registered project's geofence, mobile posts to `/api/geofence/exit-detected`, which schedules a `pending_geofence_exits` row with `fires_at = now() + 5 min`. If the user re-enters within 5 min, mobile posts to `/api/geofence/exit-cancelled` to mark it `cancelled`. Otherwise Vercel Cron (`* * * * *`) hits `/api/cron/process-pending-exits` (auth: `Authorization: Bearer ${CRON_SECRET}`), which atomically per row sets `time_entries.clock_out = now()` (audit fields `edited_at`/`edited_by_user_id` recorded as system-on-behalf-of-user) and marks the pending row `fired`. The shared executor lives in `server/lib/timesheets.ts` (`executeAutoClockOut`, accepts an optional tx). The `notes` text column on `pending_geofence_exits` does double duty: stores error text for `status='failed'` AND short tags like `"already_clocked_out"` / `"time_entry_missing"` for clean `status='fired'` rows. Auto-undo (`DELETE /api/timesheets/:id/auto-undo`) handles three cases: still-clocked-in → hard delete; recently auto-clocked-out → re-open (preserves manager audit edits via self-edit guard on `edited_by_user_id === user_id`); fallback hard-delete if within the 60-min clock-in window. Push notifications are S32b (not yet shipped). `CRON_SECRET` must be set on Vercel (and Replit for dev parity); generate via `openssl rand -hex 32`.
