...
- **Key Features**:
    ...
    - **Showcases / Public Portfolio (July 2026)**: CompanyCam-style curated portfolios. Internal pages `/showcases` (index + analytics), `/showcases/:id/edit`, `/showcases/settings`. Public pages `/p/:slug` (portfolio, `/embed` variant) and `/p/:slug/:showcaseSlug`; `/p/:slug` resolver falls back to legacy project share tokens on 404. Server module `server/showcases.ts`; public photos served only via `/api/public/showcase-img/:mediaId?s={portfolioSlug}` (sharp-resized, EXIF-stripped; anonymous access REQUIRES the portfolio slug and the media must be in a published showcase of that enabled portfolio — no cross-tenant media-id enumeration; owner-auth fallback without slug for draft previews). Public pages get the Maps key from unauthenticated `GET /api/public/maps-config` (restrict the key by referrer in Google Cloud console). Display coordinates are obfuscated server-side (~0.15–0.5 mi deterministic jitter seeded by showcase id) on save — precise coords are never persisted or exposed. OG/JSON-LD HTML injection for `/p/*` runs in production only (vercel.json rewrite `/p/(.*) → /api/index`); JSON-LD uses a `</script>`-breakout-safe serializer. Anonymous view analytics in `showcase_views` (client sessionStorage-throttled).
